...
@endif
    ...
    <div class="form-group">
        <label for="url">Immagine dell'ingrediente:</label>
            <div class="col-md-6">
                <input type="file" class="form-control @error('url') is-invalid @enderror" id="url" name="url" value="{{old('url', $ingredient->url)}}">
                @error('url')
                    <div class="invalid-feedback">{{ $message }}</div>          
                @enderror
            </div>
    ...
    <div class="mt-4">
      <button type="button" class="btn btn-dark"><a class="text-white text-decoration-none" href="{{route('admin.ingredients.index')}}">Indietro</a></button>
